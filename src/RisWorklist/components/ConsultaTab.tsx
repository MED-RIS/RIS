...
interface ConsultaTabProps {
  ...
  user: any;
  orders: Order[];
  handleCreateOrder: (e: any, extendedOrder: any) => Promise<boolean>;
  newOrder: any;
...
export default function ConsultaTab({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!newOrder.scheduledDate) {
      toast.error('Debe indicar la fecha y hora del estudio.');
      return;
    }
    if (!lines.some(line => line.serviceId || line.modality) && !directAmount) {
      toast.error('Debe seleccionar un estudio o ingresar un monto directo.');
      return;
    }
    const parsedCoverage = parseFloat(coveragePercent);
    if (hasInsurance && (!Number.isFinite(parsedCoverage) || parsedCoverage < 0 || parsedCoverage > 100)) {
      toast.error('La cobertura debe estar entre 0% y 100%.');
      return;
    }
    const selectedCompany = companies.find((item: any) => item._id === company);
    if (hasInsurance && selectedCompany?.requiresAuthorization && !authorizationNumber.trim()) {
      toast.error('Este convenio requiere un número de autorización.');
      return;
    }
    ...
    try {
      ...
      const created = await handleCreateOrder(null, extendedOrder);
      if (!created) return;
      resetForm();
      setShowSuccess(true);
